...
IVC* vc_read_image(char* filename)
{
	...
	if ((file = fopen(filename, "rb")) != NULL)
	{
		...
		if (levels == 1) // PBM
		{
			if (sscanf_s(netpbm_get_token(file, tok, sizeof(tok)), "%d", &width) != 1 ||
				sscanf_s(netpbm_get_token(file, tok, sizeof(tok)), "%d", &height) != 1)
			{
#ifdef VC_DEBUG
		...
		else // PGM ou PPM
		{
			if (sscanf_s(netpbm_get_token(file, tok, sizeof(tok)), "%d", &width) != 1 ||
				sscanf_s(netpbm_get_token(file, tok, sizeof(tok)), "%d", &height) != 1 ||
				sscanf_s(netpbm_get_token(file, tok, sizeof(tok)), "%d", &levels) != 1 || levels <= 0 || levels > 255)
			{
#ifdef VC_DEBUG
...
int vc_gray_negative(IVC* srcdst)
{
	...
	// Verificar se n�o existe erros
	if (data == NULL || width <= 0 || height <= 0) return 0;
	if (channels != 1) return 0;	// 1 canal pois est� em tons de cinzento

	// Para fazer a invers�o da imagem
...
// Inverter a imagem que est� em cores
int vc_rgb_negative(IVC* srcdst)
{
	...
	// Verificar se n�o existe erros
	if (data == NULL || width <= 0 || height <= 0) return 0;
	if (channels != 3) return 0;	// 3 canais pois est� a cores (Vermelho, Verde, Azul)
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			// Para modificar os tr�s canais (Vermelho, Verde e Azul)
			data[pos] = 255 - data[pos];
		...
		for (x = 0; x < width; x++)
		{
			...
			for (ky = -offset; ky <= offset; ky++)
			{
				for (kx = -offset; kx <= offset; kx++)
				{
					...
					{
						posk = (y + ky) * bytesperline + (x + kx) * channels;
						//verifica se o valor do pixel vizinho � 0. Se sim altera o valor do pixel
						if (data[posk] == 255)
						{
							i = 1;
						}
...
// nlabels	: Endereçoo de memória de uma variável, onde será armazenado o número de etiquetas encontradas.
...
//desenha a bounding box e o centro de massa na imagem para matricula
int vc_draw_image(OVC* blobs, IVC* src, int nblobs)
{
	...
	int xfinal, yfinal;
	long int pos_src, pos_dst, pos_aux;
	int i;
	int auxX, auxY;
	IVC* matricula;
	int pos;
	...
	for (i = 0; i <= nblobs; i++) {
		//ratio da largura sobre a altura, para que so seja identificada o blob da matricula
		
		if (blobs[i].area > 20000 && blobs[i].area < 22000)
		{
			width = blobs[i].width;
			...
			auxX = blobs[i].xc;
			auxY = blobs[i].yc;
			//matricula = vc_image_new(width, height, 3, src->levels);

			//calcula a posição do centro de massa do objeto
			pos_aux = auxY * bytesperline_src + auxX * channels_src;
			datasrc[pos_aux] = 255;
			datasrc[pos_aux + 1] = 0;
			datasrc[pos_aux + 2] = 0;
			//	xfinal = blobs[i].x + height;
				//yfinal = blobs[i].y + width;
			for (x = blobs[i].x; x <= (xinitial + width); x++)  // desenha um dos lados maiores
			{
				y = blobs[i].y;
				pos_src = y * bytesperline_src + x * channels_src;
				datasrc[pos_src] = 255;
			...
			for (y = blobs[i].y; y <= (yinitial + height); y++) // desenha um dos lados menores
			{
				x = blobs[i].x;
				pos_src = y * bytesperline_src + x * channels_src;
				datasrc[pos_src] = 255;
			...
			for (x = xinitial; x <= (xinitial + width); x++) //desenha o outro lado maior
			{
				y = (yinitial + height);
				pos_src = y * bytesperline_src + x * channels_src;
				datasrc[pos_src] = 255;
			...
			for (y = yinitial; y <= (yinitial + height); y++) // desenha o outro lado menor
			{
				x = (xinitial + width);
				pos_src = y * bytesperline_src + x * channels_src;
				datasrc[pos_src] = 255;
			...
			/*for (y2 = blobs[i].y, y3=0; y2 <= (yinitial + height), y3<height; y2++, y3++)
			{
				
				for (x2 = blobs[i].x, x3=0; x2 <= (xinitial + width), x3<width; x2++, x3++)
				{
					
					
					// Calcular posi��o
					pos = y2 * bytesperline_src + x2 * channels_src;
					pos2 = y3 * matricula->bytesperline + x3 * matricula->channels;

					//copiar a data para a nova imagem que é so a matricula
					matricula->data[pos2] =datasrc[pos];
					matricula->data[pos2 + 1] =datasrc[pos + 1];
					matricula->data[pos2 + 2] = datasrc[pos + 2];
					
				}
				
			}

			vc_write_image("t77.pgm", matricula);
			vc_image_free(matricula);
			*/
...
//desenha a bounding box e o centro de massa na imagem para letras
int vc_draw_imageL(OVC* blobs, IVC* src, int nblobs)
{
	...
	for (i = 0; i <= nblobs; i++) {
		...
		if (blobs[i].area > 100 && blobs[i].area < 200)
		{
			width = blobs[i].width;
			height = blobs[i].height;
			xinitial = blobs[i].x;
